...
import { setTeamMapPoints, setTeamMatchPoints } from "@/actions/gbx/player";
...
export default function SetTeamMapPointsForm({
  ...
  const form = useForm<SetTeamPointsSchemaType>({
    ...
    defaultValues: {
      serverId,
      team: "0",
      points: teams[0]?.mapPoints,
    },
  });

  async function onSubmit(values: SetTeamPointsSchemaType) {
    try {
      const { error } = await setTeamMapPoints(
        values.serverId,
        Number(values.team),
        values.points,
      );
      ...
      if (type === "teams") {
        const { error: error2 } = await setTeamMatchPoints(
          values.serverId,
          Number(values.team),
          values.points,
        );
  ...
  const defaultPoints = teams[Number(selectedTeam)]?.mapPoints || 0;
